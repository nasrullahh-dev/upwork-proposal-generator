...
import { Loader2, Copy, CheckCircle2, AlertCircle, WifiOff, Wifi, ArrowLeft, Save } from "lucide-react"
...
import { addProposalToStorage } from "@/lib/client-storage"
...
export default function NewProposalForm() {
    ...
    try {
      ...
      if (offlineMode) {
        ...
        if (generatedProposal === "API_KEY_MISSING") {
          setError("OpenAI API key is missing. Please add your API key or use offline mode.")
          setIsGenerating(false)
          return
        } else if (generatedProposal === "API_KEY_INVALID") {
          setError("Invalid OpenAI API key. Please update your API key or use offline mode.")
          setIsGenerating(false)
          return
  ...
  const handleSave = () => {
    ...
    try {
      ...
      addProposalToStorage(newProposal)
